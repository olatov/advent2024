...
rtl.module("webworker",["System","JS","weborworker"],function () {
  "use strict";
  var $mod = this;
  this.console = null;
});
...
rtl.module("ServiceWorkerApp",["System","Classes","SysUtils","Types","JS","weborworker","webworker","CustApp"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  rtl.createClass(this,"TServiceWorkerApplication",pas.CustApp.TCustomApplication,function () {
    this.$init = function () {
    ...
    this.$final = function () {
      ...
      pas.CustApp.TCustomApplication.$final.call(this);
    };
    this.PutInCache = async function (Request, Response) {
      var Cache = null;
      Cache = await self.caches.open(this.FCacheName);
      await Cache.put(Request,Response);
    };
    this.CacheFirst = async function (Request, PreloadResponsePromise, FallbackUrl) {
      var Result = undefined;
      var ResponseFromCache = null;
      var PreloadResponse = null;
      var ResponseFromNetwork = null;
      var FallbackResponse = null;
      Result = null;
      ResponseFromCache = await self.caches.match(Request);
      if (ResponseFromCache != null) return ResponseFromCache;
      PreloadResponse = await PreloadResponsePromise;
      if (PreloadResponse != null) {
        this.PutInCache(Request,PreloadResponse.clone());
        return PreloadResponse;
      };
      try {
        ResponseFromNetwork = await self.fetch(Request);
        this.PutInCache(Request,ResponseFromNetwork.clone());
        return ResponseFromNetwork;
      } catch ($e) {
        FallbackResponse = await self.caches.match(FallbackUrl);
        if (FallbackResponse != null) return FallbackResponse;
        Result = new Response("Network error happened",pas.JS.New(["status",408,"headers",pas.JS.New(["Content-Type","text/plain"])]));
      };
      return Result;
    };
    this.EnableNavigationPreload = async function () {
      var Result = undefined;
      Result = null;
      if (self.registration.navigationPreload) await self.registration.navigationPreload.enable();
      return Result;
    };
    this.DeleteCache = async function (key) {
      await self.caches.delete(key);
    };
    this.DeleteOldCaches = async function () {
      var $Self = this;
      var Result = undefined;
      var CacheKeepList = [];
      var CachesToDelete = null;
      var KeyList = null;
      CacheKeepList = [this.FCacheName];
      KeyList = await self.caches.keys();
      CachesToDelete = KeyList.filter(function (key, index, anArray) {
        var Result = false;
        Result = !CacheKeepList.includes(key);
        return Result;
      });
      Result = await Promise.all(CachesToDelete.map(rtl.createCallback($Self,"DeleteCache")));
      return Result;
    };
    this.SetFallbackURL = function (AValue) {
      if (this.FFallbackURL === AValue) return;
      this.FFallbackURL = AValue;
    };
    this.DoRun = function () {
      self.addEventListener("activate",rtl.createCallback(this,"Activate"));
      self.addEventListener("install",rtl.createCallback(this,"Install"));
      self.addEventListener("fetch",rtl.createCallback(this,"Fetch"));
    };
    this.Activate = function (Event) {
      Event.waitUntil(this.EnableNavigationPreload());
      Event.waitUntil(this.DeleteOldCaches());
    };
    this.Install = function (Event) {
      var $Self = this;
      Event.waitUntil(self.caches.open(this.FCacheName).then(function (Cache) {
        Cache.addAll(rtl.arrayRef($Self.FResources));
      }));
    };
    this.Fetch = function (FetchEvent) {
      FetchEvent.respondWith(this.CacheFirst(FetchEvent.request,FetchEvent.preloadResponse,this.FFallbackURL));
    };
    this.ShowException = function (E) {
      ...
      pas.webworker.console.log("Unhandled exception caught:" + S);
    };
    this.HandleException = function (Sender) {
  ...
  $mod.$implcode = function () {
    $impl.EnvNames = null;
    $impl.ReloadEnvironmentStrings = function () {
      var I = 0;
      var S = "";
      var N = "";
      var A = [];
...
rtl.module("program",["System","Classes","ServiceWorkerApp"],function () {
  "use strict";
  var $mod = this;
  this.Yourcachename = "v1";
  rtl.createClass(this,"Tapplication",pas.ServiceWorkerApp.TServiceWorkerApplication,function () {
    this.Create$1 = function (Aowner) {
      pas.CustApp.TCustomApplication.Create$1.call(this,Aowner);
      this.FCacheName = $mod.Yourcachename;
      this.FResources = ["images/error.png"];
      this.SetFallbackURL("images/error.png");
      return this;
    };
    var $r = this.$rtti;
    $r.addMethod("Create$1",2,[["Aowner",pas.Classes.$rtti["TComponent"]]]);
  ...
  this.App = null;
  $mod.$main = function () {
    $mod.App = $mod.Tapplication.$create("Create$1",[null]);
    $mod.App.Run();
  };
});
rtl.run();
